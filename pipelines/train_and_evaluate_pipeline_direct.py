"""
Training and evaluation pipeline for the direct pick-and-place task.
Uses pickAndPlaceDirect.xml and trajectoriesDirect.py.
Robot goes directly to block, picks it up, then uses circular path to place it.
...
def train_and_evaluate_step(target_trajectories, states, actions, traj_ids, steps,
    ...
    return train_and_evaluate_step_base(
        ...
        batch_size, model_name_prefix='direct', MLP_class=MLP,
...
# State encoding flag for direct (does NOT use sin/cos encoding)
USE_SIN_COS_ENCODING = False

# load_and_filter_dataset, get_batch_size, and load_or_create_model are now imported from training_utils
...
class MLP(nn.Module):
    """MLP for direct pick-and-place task - larger architecture"""
...
def evaluate_model(model, num_objects=20, max_steps=None, state_scaler=None, action_scaler=None):
    ...
    """
    # Lambda wrapper that uses initialize_block_random_orientation but passes block_rotation to sample_block_pos
    def initialize_block_direct(model, data, block_geom_id, block_jnt_id):
        # Compute rotation Q first (same logic as initialize_block_random_orientation)
        axes = np.eye(3)
        ...
        # Wrap sample_block_pos to pass block_rotation
        def wrapped_sample_block_pos():
            block_size = sample_block_size()
            return sample_block_pos(block_size=block_size, block_rotation=Q)
        
        # Use utility function with wrapped sample_block_pos
        return initialize_block_random_orientation(
            model, data, block_geom_id, block_jnt_id,
            wrapped_sample_block_pos, sample_block_size
        )
    
    # Define action application callback for absolute actions
    def apply_absolute_action(model_output, data, model_mj, site_id, block_id, dof_ids, actuator_ids, gripper_actuator_id):
        action = model_output
        
        # Apply action (use larger scale for faster convergence)
        # The model outputs absolute joint positions, so we can apply them more directly
        action_scale = 1.1
    ...
    return evaluate_model_unified(
        ...
        initialize_block_direct,
        USE_SIN_COS_ENCODING, apply_absolute_action, placement_box_pos, placement_box_size
    )


def collectDataCSV_direct(target_successes=500, base_filename='dataset_direct', trajectories_per_file=200, save_frequency=20):
    """
    Collect trajectories using trajectoriesDirect.py and save to CSV.
    ...
    collect_trajectory_data_unified(
        ...
        initialize_block_fn=initialize_block_direct,
...
def main():
    # Use different filenames for direct version
    base_filename = 'dataset_direct'
    variation = get_variation_from_base_filename(base_filename)
    dataset_dir = get_dataset_path(variation)
    os.makedirs(dataset_dir, exist_ok=True)
    preprocessed_file = os.path.join(dataset_dir, 'dataset_direct_preprocessed.pkl')
    variation_dir = get_variation_path(variation)
    os.makedirs(variation_dir, exist_ok=True)
    results_file = os.path.join(variation_dir, 'training_results_direct.json')
    results_csv = os.path.join(variation_dir, 'training_results_direct.csv')
    ...
    for target_trajectories in data_sizes:
        ...
        # Load and filter dataset (50% normal + 50% recovery data if available)
        print(f"\nLoading and filtering dataset...")
        recovery_filename = 'dataset_direct_recovery'
        ...
        # Get joint ranges for action normalization (get from model once)
        if 'joint_ranges' not in locals() or joint_ranges is None:
            model_mj_temp, _, _, _, _, dof_ids_temp, _, _, _, _ = setup_mujoco_model(XML_PATH, dt)
            joint_ranges = get_joint_ranges(model_mj_temp, dof_ids_temp)
            print(f"  Loaded joint ranges for action normalization")
        
        # Get batch size and train/evaluate (auto-detect device for batch size)
        # Auto-detect device for batch size calculation
        if hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
            device_type = 'mps'
        elif torch.cuda.is_available():
            device_type = 'cuda'
