"""
Training and evaluation pipeline for pick-and-place using task-space absolute actions.
Uses pickAndPlaceSimpler.xml and trajectoriesTaskSpace.py.
...
def train_and_evaluate_step(target_trajectories, states, actions, traj_ids, steps,
    ...
    return train_and_evaluate_step_base(
        ...
        batch_size, model_name_prefix='taskspace_absolutes', MLP_class=MLP,
...
class MLP(nn.Module):
    """MLP for task-space pick-and-place task - larger architecture"""
...
def collectDataCSV_taskspace_absolutes(target_successes=500, base_filename='dataset_taskspace_absolutes', trajectories_per_file=200, save_frequency=20):
    """
    Collect trajectories using trajectoriesTaskSpace.py and save to CSV.
...
def main():
    # Use taskspace delta datasets (will convert to absolutes during loading)
    base_filename = 'dataset_taskspace'  # Use existing delta datasets
    variation = get_variation_from_base_filename('dataset_taskspace_absolutes')  # Use absolutes variation for output
    dataset_dir = get_dataset_path(variation)
    os.makedirs(dataset_dir, exist_ok=True)
    preprocessed_file = os.path.join(dataset_dir, 'dataset_taskspace_absolutes_preprocessed.pkl')
    variation_dir = get_variation_path(variation)
    os.makedirs(variation_dir, exist_ok=True)
    results_file = os.path.join(variation_dir, 'training_results_taskspace_absolutes.json')
    results_csv = os.path.join(variation_dir, 'training_results_taskspace_absolutes.csv')
    ...
    for target_trajectories in data_sizes:
        ...
        # Check current dataset size (look in taskspace datasets, but save to absolutes)
        # Load from taskspace datasets (the source)
        files = find_dataset_files('dataset_taskspace', pattern='*.csv')
        ...
        # Note: We use the delta dataset, so if we need more data, we'd collect delta data
        # For now, we'll just use existing delta datasets
        if num_trajectories_total < target_trajectories:
            ...
            print(f"⚠️  Note: Using existing taskspace delta datasets. If you need more data,")
            print(f"   collect it using train_and_evaluate_pipeline_taskSpace.py first.")
            # Don't collect new data - just use what we have
            if num_trajectories_total == 0:
                print(f"  ✗ No data found! Please collect data first.")
                continue
        
        # Load and filter dataset (50% normal + 50% recovery data)
        print(f"\nLoading and filtering dataset from taskspace delta datasets...")
        recovery_filename = 'dataset_taskspace_recovery'  # Use delta recovery dataset
        states_subset, actions_delta_subset, trajectory_ids_subset, steps_subset = load_and_filter_dataset(
        ...
        print(f"  Converting {len(actions_delta_subset)} delta actions to absolute actions...")
        actions_subset = np.array([
            convert_delta_action_to_absolute_action(state, delta_action, USE_SIN_COS_ENCODING)
            for state, delta_action in zip(states_subset, actions_delta_subset)
        ])
        print(f"  ✓ Converted to absolute actions")
        
        # Print filtering info
        unique_traj_ids_subset = np.unique(trajectory_ids_subset)
        print(f"  ✓ Using {len(states_subset)} state-action pairs from {len(unique_traj_ids_subset)} unique trajectories (converted from delta)")
