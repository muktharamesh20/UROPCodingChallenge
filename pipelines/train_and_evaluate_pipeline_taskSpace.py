"""
Training and evaluation pipeline for pick-and-place using task-space delta actions.
...
Uses task-space delta actions (end-effector pose deltas) instead of joint deltas.
Actions: [delta_ee_pos(3), delta_ee_quat(3), gripper(1)] = 7D
"""
import sys
import os
...
from utils.training_utils import (
    ...
    extract_action_taskspace_delta,
...
def train_and_evaluate_step(target_trajectories, states, actions, traj_ids, steps,
    ...
    return train_and_evaluate_step_base(
        ...
        batch_size, model_name_prefix='taskspace', MLP_class=MLP,
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.fc5(x)
        # No clamping - task-space deltas can be positive or negative
        return x
...
def evaluate_model(model, num_objects=20, max_steps=None, state_scaler=None, action_scaler=None):
    ...
    """
    # Define action application callback for task-space delta actions
    def apply_taskspace_delta_action(model_output, data, model_mj, site_id, block_id, dof_ids, actuator_ids, gripper_actuator_id):
        ...
        desired_pos, desired_quat = compute_desired_pose_from_delta(
            data, site_id, delta_ee_pos, delta_ee_quat, delta_scale=delta_scale
        )
        
        # Determine gripper state (use previous state or infer from action)
    ...
    return evaluate_model_unified(
        ...
        USE_SIN_COS_ENCODING, apply_taskspace_delta_action, placement_box_pos, placement_box_size
    )

def collectDataCSV_taskspace(target_successes=500, base_filename='dataset_taskspace', trajectories_per_file=200, save_frequency=20):
    """
    Collect trajectories using trajectoriesTaskSpace.py and save to CSV.
    Actions are task-space deltas: [delta_ee_pos(3), delta_ee_quat(3), gripper(1)] = 7D
    
    Uses full quaternions from trajectories directly (no z-axis-only modification).
    moveArm will handle IK conversion naturally.
    Uses unified trajectory collection function.
    """
    # Define action extraction callback for task-space delta actions
    def extract_action_taskspace_delta_callback(data, model, site_id, block_id, dof_ids, actuator_ids, gripper_actuator_id,
                                                 desiredPos, desiredQuat, trajectory, i,
                                                 current_qpos_before=None, current_ee_pos_before=None, current_ee_quat_before=None):
        # Use full quaternion from trajectory directly (no z-axis-only modification)
        return extract_action_taskspace_delta(
            current_ee_pos_before, current_ee_quat_before,
    ...
    collect_trajectory_data_unified(
        ...
        extract_action_fn=extract_action_taskspace_delta_callback,
...
def main():
    # Use different filenames for task-space version
    base_filename = 'dataset_taskspace'
    variation = get_variation_from_base_filename(base_filename)
    dataset_dir = get_dataset_path(variation)
    os.makedirs(dataset_dir, exist_ok=True)
    preprocessed_file = os.path.join(dataset_dir, 'dataset_taskspace_preprocessed.pkl')
    variation_dir = get_variation_path(variation)
    os.makedirs(variation_dir, exist_ok=True)
    results_file = os.path.join(variation_dir, 'training_results_taskspace.json')
    results_csv = os.path.join(variation_dir, 'training_results_taskspace.csv')
    ...
    for target_trajectories in data_sizes:
        ...
        recovery_filename = 'dataset_taskspace_recovery'
