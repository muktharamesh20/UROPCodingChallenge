...
Uses pickAndPlaceSimpler.xml and trajectoriesTaskSpaceAbsolutesRandomStart.py.
...
from trajectories.trajectoriesTaskSpaceAbsolutesRandomStart import (
    createTrajectory,
    sample_block_pos,
    sample_block_size,
...
def train_and_evaluate_step(target_trajectories, states, actions, traj_ids, steps,
    ...
    return train_and_evaluate_step_base(
        ...
        batch_size, model_name_prefix='taskspace_absolutes_random_start', MLP_class=MLP,
...
class MLP(nn.Module):
    """MLP for task-space pick-and-place task with random starts - larger architecture"""
...
def collectDataCSV_taskspace_absolutes_random_start(target_successes=500, base_filename='dataset_taskspace_absolutes_random_start', trajectories_per_file=200, save_frequency=20):
    """
    Collect trajectories using trajectoriesTaskSpaceAbsolutesRandomStart.py and save to CSV.
...
def main():
    # Use taskspace_absolutes_random_start variation
    base_filename = 'dataset_taskspace_absolutes_random_start'
    variation = get_variation_from_base_filename(base_filename)
    dataset_dir = get_dataset_path(variation)
    os.makedirs(dataset_dir, exist_ok=True)
    preprocessed_file = os.path.join(dataset_dir, 'dataset_taskspace_absolutes_random_start_preprocessed.pkl')
    variation_dir = get_variation_path(variation)
    os.makedirs(variation_dir, exist_ok=True)
    results_file = os.path.join(variation_dir, 'training_results_taskspace_absolutes_random_start.json')
    results_csv = os.path.join(variation_dir, 'training_results_taskspace_absolutes_random_start.csv')
    ...
    for target_trajectories in data_sizes:
        ...
        if num_trajectories_total < target_trajectories:
            ...
            collectDataCSV_taskspace_absolutes_random_start(
        ...
        if use_finetuning:
            ...
            if explicit_pretrained_model and os.path.exists(explicit_pretrained_model):
                ...
                if prev_sizes:
                    ...
                    prev_models = find_model_file(f"model_taskspace_absolutes_random_start_{prev_size}traj_*.pth", base_name=variation)
                    if prev_models:
