...
Uses smaller model architecture (512→256→128) to prevent overfitting.
"""
import sys
import os
...
def train_and_evaluate_step(target_trajectories, states, actions, traj_ids, steps,
    ...
    return train_and_evaluate_step_base(
        ...
        batch_size, model_name_prefix='taskspace_random_smaller', MLP_class=MLP,
...
class MLP(nn.Module):
    """MLP for task-space pick-and-place task with random starts - smaller architecture to prevent overfitting"""
...
def main():
    # Use taskspace_random_smaller variation
        ...
        if use_finetuning:
            ...
            if explicit_pretrained_model and os.path.exists(explicit_pretrained_model):
                ...
                if prev_sizes:
                    ...
                    prev_models = find_model_file(f"model_taskspace_random_smaller_{prev_size}traj_*.pth", base_name=variation)
                    if prev_models:
