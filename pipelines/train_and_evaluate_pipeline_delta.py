"""
Training and evaluation pipeline for pick-and-place using delta actions.
...
Uses delta actions (joint position deltas) instead of absolute positions.
"""
import sys
import os
...
def train_and_evaluate_step(target_trajectories, states, actions, traj_ids, steps,
    ...
    return train_and_evaluate_step_base(
        ...
        batch_size, model_name_prefix='delta', MLP_class=MLP,
...
# State encoding flag for delta pipeline (uses sin/cos encoding)
...
def evaluate_model(model, num_objects=20, max_steps=None, state_scaler=None, action_scaler=None):
    ...
    """
    # Define action application callback for delta actions
...
def collectDataCSV_simpler(target_successes=500, base_filename='dataset_delta', trajectories_per_file=200, save_frequency=20):
    """
    Collect trajectories using trajectoriesEvenSimpler.py and save to CSV.
    Uses unified trajectory collection function.
    """
    # Define action extraction callback for delta actions
    def extract_action_delta_callback(data, model, site_id, block_id, dof_ids, actuator_ids, gripper_actuator_id,
                                       desiredPos, desiredQuat, trajectory, i,
                                       current_qpos_before=None, current_ee_pos_before=None, current_ee_quat_before=None):
        # moveArm has already been called, so desired_qpos is in data.ctrl
        desired_qpos = data.ctrl[actuator_ids].copy()
        return extract_action_delta(current_qpos_before, desired_qpos, data.ctrl, gripper_actuator_id)
    ...
    collect_trajectory_data_unified(
        ...
        extract_action_fn=extract_action_delta_callback,
...
def main():
    # Use different filenames for delta version
    base_filename = 'dataset_delta'
    variation = get_variation_from_base_filename(base_filename)
    dataset_dir = get_dataset_path(variation)
    os.makedirs(dataset_dir, exist_ok=True)
    preprocessed_file = os.path.join(dataset_dir, 'dataset_delta_preprocessed.pkl')
    variation_dir = get_variation_path(variation)
    os.makedirs(variation_dir, exist_ok=True)
    results_file = os.path.join(variation_dir, 'training_results_delta.json')
    results_csv = os.path.join(variation_dir, 'training_results_delta.csv')
    ...
    for target_trajectories in data_sizes:
        ...
        recovery_filename = 'dataset_delta_recovery'
