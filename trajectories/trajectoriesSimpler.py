...
def sample_block_pos():
    ...
    while True:
        ...
        on_placement_box = (placement_box_pos[0] - np.abs(placement_box_size[0]) <= x <= placement_box_pos[0] + np.abs(placement_box_size[0]) and
                           placement_box_pos[1] - np.abs(placement_box_size[1]) <= y <= placement_box_pos[1] + np.abs(placement_box_size[1]))
...
# Helper functions
from utils.ik_utils import interpolate, slerp, computeIKError, computeBlockGraspInfo, moveArm, verifyPlacement
...
def sample_robot_start_position(model, dof_ids, home_key_id=None):
    ...
    # Joint 0 (first joint) always stays at default/home position
    for idx, jid in enumerate(dof_ids):
        # Skip joint 0 - keep it at default/home position
        if idx == 0:
            continue
...
# computeIKError is now imported from ik_utils
